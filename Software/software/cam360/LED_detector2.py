...
class LEDDetector:
    ...
    def __init__(self):
        ...
        self.color_dict = {"blue_low": np.array([100, 120, 200]), "blue_high": np.array([130, 255, 255]),
                           "yellow_low": np.array([10, 2, 150]), "yellow_high": np.array([60, 40, 255]),
    ...
    def moment_search(self, verbose=False, visualisation=False):
        """
        ...
        try:
            for led in self.led_list:
                ...
                if num_contours == 0:
                    ...
                    if max_area >= 3.0:
                        ...
                        if visualisation:
                            cv2.circle(self.frame, tuple(led.position), 30, led.color_tuple, -1)  # draw filled circle
            ...
            if visualisation:
                ...
                print('blue mask: ')
                visualize([self.blue.mask])
                #visualize([self.blue.img_color])
                print('green mask: ')
                visualize([self.green.mask])
                #visualize([self.green.img_color])
                print('yellow mask: ')
                visualize([self.yellow.mask])
                #visualize([self.yellow.img_color])
                print('red mask: ')
                visualize([self.red.mask])
                #visualize([self.red.img_color])
    ...
    def line_search(self, verbose=False, visualisation=False):
        ...
        for led in self.led_list:
            ...
            #led.edge = cv2.Canny(led.mask_processed, 10, 200)
            if visualisation:
    ...
    def flip(self, visualisation=False):
        ...
        #              (self.camera_center[0], self.camera_center[1]-200), (0,255,255))
        #     cv2.line(self.frame, (self.camera_center[0] + 200, self.camera_center[1]),
        #              (self.camera_center[0] - 200, self.camera_center[1]), (0, 255, 255))
        #     cv2.line(self.frame, (int(self.windowsize[0]/2), int(self.windowsize[1]/2) + 100),
        #              (int(self.windowsize[0]/2), int(self.windowsize[1]/2) - 100), (0, 0, 255))
        #     cv2.line(self.frame, (int(self.windowsize[0]/2) + 100, int(self.windowsize[1]/2)),
        #              (int(self.windowsize[0]/2) - 100, int(self.windowsize[1]/2)), (255, 0, 255))
        #     visualize([self.frame])
        ...
        # flip center as well!
        # print('before: ', self.camera_center[0], type(self.camera_center))
        # print(self.windowsize[0], type(self.windowsize))
        self.camera_center[0] = self.windowsize[1] - self.camera_center[0]
        # print('after: ', self.camera_center[0])
    ...
    def validate_led_position(self, led_list, verbose=False, visualisation=False):
        ...
        if len(sorted_list) == 4:
            ...
            if False:
                ...
                rand_num = getrandbits(1)
                excluded = sorted_list[(idx + rand_num) % len(sorted_list)].color
                sorted_list.pop((idx + rand_num) % len(sorted_list))
                print(excluded, ' LED popped --> continue with 3 leds: ')
    ...
    def detect(self, verbose=False, visualisation=False):
        ...
        self.moment_search(verbose=verbose, visualisation=visualisation)
...
led_det = LEDDetector()
img1 = cv2.imread('example_photos/arena/test6/3.jpg')
img2 = cv2.imread('example_photos/arena/test5/3.jpg')
img3 = cv2.imread('example_photos/arena/test4/3.jpg')
img4 = cv2.imread('example_photos/arena/test8/3.jpg')
...
led_det.color_mask(visualisation=True)
led_det.process(kernel_size=4, mode='doddq')


#led_det.line_search(verbose=True, visualisation=True)
led_det.moment_search(verbose=True, visualisation=True)
