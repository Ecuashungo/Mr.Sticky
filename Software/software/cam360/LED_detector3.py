...
class LEDDetector:
    ...
    def get_frame(self, verbose=False, visualisation=False):
        ...
        if verbose:
            if self.frame is None:
                ...
                # print('self.frame after reading from picam: ', self.frame)
    ...
    def moment_search(self, verbose=False, visualisation=False, presentation=False, save=False):
        ...
        if save:
            ...
            #cv2.imwrite('frames/' + str(self.counter) + '.bmp', self.frame)
        ...
        try:
            ...
            if save:
                ...
                #cv2.imwrite('frames_modified/' + str(self.counter) + '.bmp', self.frame)
    ...
    def detect(self, verbose=False, visualisation=False, presentation=False, save=False):
        ...
        if save and self.position != [None, None, None]:
            ...
            l = 20
            color = (30, 30, 255)
            #arena = cv2.imread("arena.png", cv2.IMREAD_COLOR)
    ...
    def presentation(self):
        ...
        #l = 5
        #color = (30, 30, 255)
        #arena = cv2.imread("arena.jpg", cv2.IMREAD_COLOR)
        #arena = cv2.resize(arena, (400, 400)) 
        #cv2.imshow('arena', arena)
        #print('self.position: ', self.position)
        #cv2.arrowedLine(arena,(self.position[0], self.position[1]), (self.position[0] + l * m.sin(self.position[2]),self.position[1] + l * m.cos(self.position[2])), color)
        print('after arena')
        # 360 cam
        for led in self.led_list:
        ...
        # FIXME: add map with actual position
